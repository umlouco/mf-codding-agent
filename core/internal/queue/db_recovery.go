...
//
// BLOCKED is deliberately exempt from the rewrite: it is the one sanctioned
// terminal exit for a row the queue cannot complete, including the
// `decomposition_*` rows this invariant guards. Without the exemption the
// trigger forces `status = 'BLOCKED'` back to 'VERIFYING', so the extension's
// blockTask can never commit, the supervisor re-decides the same task every
// tick, and the whole queue is head-of-line blocked behind it. A human-blocked
// row is not an accepted failure, so it does not revive the failed parent.
