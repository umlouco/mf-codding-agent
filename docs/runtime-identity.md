...
| Working roles and context | [src/queue/agents.ts](../src/queue/agents.ts) supplies configured tool definitions to supervisors as well as workers, preserving configured memory and MCP tools. [src/queue/claudeCli.ts](../src/queue/claudeCli.ts) retains available CLI tools. [src/queue/verification.ts](../src/queue/verification.ts) starts a fresh verifier. | Roles guide the work and context supplied to each model. Supervisors can discover inspection tools to resolve conflicting evidence. Verifier independence still relies on how checks are performed and reported. |
