...
function load(file, dependencies = {}, extra = '') {
  ...
  vm.runInNewContext(outputText, {
    ...
    require: name => {
      ...
      if (/^\.\/(db|failure|orchestrator|scope|recovery|workInventory|verificationAuthority|verificationRecovery|playwrightPolicy)/.test(name)) return load('src/queue/' + name.slice(2) + '.ts', dependencies);
